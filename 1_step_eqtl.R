...
genelist=read.table("id.txt",header = T,sep = "\t")
...
expo_rt<- read_exposure_data(
  filename = paste0("eqtlclump/",i,".txt"),
  sep = "\t",
  snp_col = "SNP",
  beta_col = "beta",
  se_col = "se",
...
result_or$FDR=p.adjust(result_or$pval,method = "fdr",n=15695)
filename=paste0("eqtlresult/",i)
dir.create(filename)
...
write.table(result,"eqtl_result.txt",sep = "\t",quote = F,row.names = F)
